...
//     var useOnlyText = true; // true - анимация загрузки текстом, false - показать картинку
//       var message = 'Please wait'; // сообщение юзеру
//       var imageSize = '30'; // размер изображения загрузки
...
//       if(useOnlyText)
//           ntf.innerHTML = '<center><div id="loading" style="width: 100%; min-height: 50px; color: #606266; letter-spacing: 2px; text-transform: uppercase; background-color: #e7e7e7; border-radius: 100px; margin-top: 20px; margin-bottom: 0; padding: 14px 20px; font-size: 16px; font-weight: 700; position: relative;">'+message+'</div></center>';
//       else
//           ntf.innerHTML = '<center><img src="data:image/png;base64,'+imageBase64+'" width="'+imageSize+'" height="'+imageSize+'"><br>'+message+'</center>';
      
//       submit.style = 'display:none';
//       submit.parentNode.insertBefore(ntf, submit.nextSibling)
      
//       if(useOnlyText){
//           var anim_js = document.createElement('script');
//           anim_js.innerHTML = 
//               `var count = 0;
...
//                 count++;
//                 el.innerHTML = el_text + new Array(count % 5).join('.');
//               }, 300);`;
          
//           document.body.appendChild(anim_js);
//       }
//   }
...
//   }
        ...
        function startCountdown(minutes) {
            ...
            function updateTimer() {
                ...
                displayMin.textContent = String(minutesLeft).padStart(2, '0');
                displaySec.textContent = String(secondsLeft).padStart(2, '0');

                if (totalSeconds <= 0) {
                    clearInterval(intervalId);
                    // Restart the countdown timer
                    setTimeout(() => startCountdown(minutes), 1000);
                } else {
                    totalSeconds--;
                }
            }
        ...
        // Start the countdown with an initial value of 3 minutes
        startCountdown(7);
